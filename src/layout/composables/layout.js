...
export function useLayout() {
    ...
    const setBackendTheme = (data) => {
        ...
        // B. Update State
        if (data.primary) layoutConfig.primary = data.primary;
        if (data.surface) layoutConfig.surface = data.surface;
        if (typeof data.darkTheme !== 'undefined') {
            layoutConfig.darkTheme = data.darkTheme;
            document.documentElement.classList.toggle('app-dark', data.darkTheme);
        }
        ...
        if (layoutConfig.surface) {
            const surfaceObj = surfaces.value.find((s) => s.name === layoutConfig.surface);
            if (surfaceObj) {
